...
function activity5() {
    ...
    calculate_table3();
    console.log(table3);
    table3_col_headings = ["Sr No.", "T<sub>B</sub> (K)", "P<sub>2</sub> (kPa)", `1/T<sub>B</sub> (K<sup>-1</sup>)`, `ln(P<sub>2</sub>)`, `ln(P<sub>2</sub>)/T<sub>B</sub>`, `Check`];
    let table3_verify_row = [["1", `${table3[0][1]}`, `${table3[0][2]}`, `<input type="text"  id="inp-1">`, `<input type="text"  id="inp-2">`, `<input type="text"  id="inp-3">`, `<input type="submit" class="btn btn-primary" onclick="verify_table3();">`]];
...
function calculate_table3() {
    for (let i = 0; i < main_table_data.length; i++) {
        table3.push([(i + 1).toString(), main_table_data[i][3].toFixed(2), main_table_data[i][2].toFixed(3), (1 / main_table_data[i][3]).toFixed(5), Math.log(main_table_data[i][2]).toFixed(4), (Math.log(main_table_data[i][2]) / main_table_data[i][3]).toFixed(5)]);
        table3_precise.push([i + 1, main_table_data[i][3], main_table_data[i][2], (1 / main_table_data[i][3]), Math.log(main_table_data[i][2]), Math.log(main_table_data[i][2]) / main_table_data[i][3]]);
    }
}
...
function verify_sigma() {
    let val1 = document.getElementById(`sigma-1`);
    let val2 = document.getElementById(`sigma-2`);
    let val3 = document.getElementById(`sigma-3`);
    console.log(parseFloat(val1.value), parseFloat(table3[0][2]));
    ...
    pp.showdescription("Your Values are Correct!!", 3);
