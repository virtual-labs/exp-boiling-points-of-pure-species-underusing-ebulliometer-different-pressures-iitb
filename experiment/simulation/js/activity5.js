...
function activity5() {
    ...
    pp.clearrightpannel();
    pp.addoffcanvas(3);
    pp.showtitle(`<div id='exp-title'>To determine the Boiling point of pure species under different pressures</div>`, 3);
    calculate_table3();
    console.log(table3);
    table3_col_headings = [
        'Sr No.',
...
function calculate_table3() {
    for (let i = 0; i < main_table_data.length; i++) {
        table3.push([
            ...
            (Math.log(main_table_data[i][2]) / main_table_data[i][3]).toFixed(5),
        ]);
        table3_precise.push([
            i + 1,
            main_table_data[i][3],
...
function verify_sigma() {
    ...
    pp.showdescription(`<div class='discription_text'>Your Values are Correct!!</div>`, 3);
