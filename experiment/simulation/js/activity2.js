...
function activity2() {
    ...
    pp.addoffcanvas(3);
    pp.showtitle(`<p id="exp-title">Figure shows the experimental setup. The assembled setup will be used to carry out the experimentation to determine the boiling point of pure species under different pressure.</p>`, 3);
    pp.showdescription(`<p class='discription_text'>Observe the assembly. Next task is to <span style="color:#018fc3">Assemble it yourself.</span></p>`, 3);
    show_panel(3);
    canvas = pp.canvas;
    ...
    scene = new Scene();
    pp.addtorightpannel(`<button id="panel2_btn" class="btn btn-primary" onclick="a2_first()" style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;">Next</button>`, 3);
    var assm = new Chemistry.Custome_image(assembly, new Chemistry.Point(900, 450), 594, 701, canvas);
    scene.add(assm);
    // add canvas sizing
...
function a2_first() {
    ...
    canvas.style.cursor = 'crosshair';
    rect = canvas.getBoundingClientRect();
    pp.addtorightpannel(`<button id="panel2_btn" class="btn btn-primary" onclick="a2_second()" style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;">Start</button>`, 3);
    scene = new Scene();
    scene1 = new Scene();
    fixed_container();
...
function a2_draw_all_components() {
    var sq = new Chemistry.Custome_image(base, new Chemistry.Point(400, 375), 594, 701, canvas);
    sq.name = 'stand';
    sq.lock();
    scene.add(sq);
    var sq = new Chemistry.Custome_image(pump_small, new Chemistry.Point(347, 85.5), 87, 106, canvas);
    sq.name = 'pump';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(pipe1_small, new Chemistry.Point(270, 350), 97, 486, canvas);
    sq.name = 'pipe1';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(pipe2_small, new Chemistry.Point(298, 350), 73, 458, canvas);
    sq.name = 'pipe2';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(condenser_small, new Chemistry.Point(340, 535), 64, 232, canvas);
    sq.name = 'condenser';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(connector_small, new Chemistry.Point(340, 410), 44, 35, canvas);
    sq.name = 'connector';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(ebulliometer_small, new Chemistry.Point(385, 280), 113, 336, canvas);
    sq.name = 'ebulliometer';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(temp_indicator_small, new Chemistry.Point(440, 370), 77, 92, canvas);
    sq.name = 'temp_indicator';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(heating_tap_small, new Chemistry.Point(405, 225), 35, 66, canvas);
    sq.name = 'heating_tap';
    scene.add(sq);
    var sq = new Chemistry.Custome_image(dimmer_stat_small, new Chemistry.Point(500, 180), 59, 59, canvas);
    sq.name = 'dimmer_stat';
    scene.add(sq);
}
...
function a2_display_msg() {
    if (current_msg_i >= a2_msg.length) {
        ...
        pp.showdescription(`<p class='discription_text' > ${a2_msg[current_msg_i]} </p>`, 3);
    }
}
