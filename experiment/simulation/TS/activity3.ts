...
var control_panel_text: Chemistry.Geo_Text;
var show_once = true;
let ob_data = [];
...
var panel: Chemistry.Rectangle;
var ebu_in: Chemistry.Circle;
var ebu_out: Chemistry.Circle;
var water_pump: Chemistry.Circle;
var n2_gas: Chemistry.Circle;
var heating_coil: Chemistry.Circle;

var drops: Chemistry.Custome_image;
var time: Chemistry.anim_image;
var point: Chemistry.Custome_image;
...
function activity3() {
	...
	pp.showtitle(
		`<p id='exp-title'> Perform task to simulate the experiment </p>`,
		3
	);
	...
	control_panel_text = new Chemistry.Geo_Text(
		'Control Panel',
		new Chemistry.Point(1500, 820),
		canvas
	);
	...
	var first_geo = new Chemistry.Custome_image(
		seq1_img,
		new Chemistry.Point(470, 440),
		594,
		701,
		canvas
	);
...
function control_panel() {
	panel = new Chemistry.Rectangle(
		400,
		500,
		new Chemistry.Point(1400, 300),
		canvas
	);
	panel.color = '#ADC4CE';
	...
	water_pump = new Chemistry.Circle(
		new Chemistry.Point(1720, 540),
		18,
		canvas
	);
	...
	heating_coil = new Chemistry.Circle(
		new Chemistry.Point(1720, 410),
		18,
		canvas
	);
...
function draw_seq_all() {
	...
	if (seq == 0) {
		...
		pp.showdescription(
			`<p class='discription_text'>Open outlet of ebulliometer.</p>`,
			3
		);
	}
	...
	if (seq == 1 && seq_container[1].l < seq_container[1].l_last) {
		...
		if ((seq_container[1].name = 'third')) {
			...
			var second_geo = new Chemistry.anim_image(
				seq5_img,
				new Chemistry.Point(470, 440),
				594,
				701,
		...
		if ((seq_container[1].name = 'fourth')) {
			...
			draw_seq_all();
			pp.showdescription(
				`<p class='discription_text'>Close inlet of ebulliometer.</p>`,
				3
			);
			show_panel(3);
		}
		...
		if ((seq_container[1].name = 'fifth')) {
			...
			var second_geo_y = new Chemistry.anim_image_y_dir_down(
				seq8_img,
				new Chemistry.Point(470, 440),
				594,
				701,
		...
		if ((seq_container[1].name = 'eighth')) {
			...
			pp.showdescription(
				`<p class='discription_text'>Turn on heating coil.</p>`,
				3
			);
...
function a3_mouseclick_ebu_out(e: MouseEvent) {
	...
	if (ebu_out.isinside(new Chemistry.Point(x, y))) {
		...
		if (ebu_out.color == 'red') {
			...
			pp.showdescription(
				`<p class='discription_text'>Turn on n2 gas.</p>`,
				3
			);
			show_panel(3);
			...
			pp.showdescription(
				`<p class='discription_text'>Open inlet of ebulliometer.</p>`,
				3
			);
			show_panel(3);
			canvas.addEventListener('click', a3_mouseclick_ebu_in);
...
function a3_mouseclick_n2(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	console.log(x, y);
	var settime = 10;
	var timeoutID;
	let sys: HTMLSelectElement = <HTMLSelectElement>(
		document.getElementById('compounds-dd')
	);
	...
	if (n2_gas.isinside(new Chemistry.Point(x, y))) {
		...
		if (n2_gas.color != 'red') {
			...
			load_system();
			pp.showdescription(
				`<p class='discription_text'>Select the system.</p>`,
				3
			);
			show_panel(3);
		...
		time = new Chemistry.anim_image(
			timer,
			new Chemistry.Point(1050, 700),
			290,
			210,
			canvas
		);
		...
		var time_text = new Chemistry.Geo_Text(
			`${
				settime == 10 ? settime.toString() : 0 + settime.toString()
			} sec`,
			new Chemistry.Point(1045, 710),
			canvas
		);
		...
		if (settime == 10) {
			...
			timeoutID = setTimeout(() => {
				...
				if (settime >= 0) {
					...
					canvas.addEventListener('click', a3_mouseclick_n2);
					pp.showdescription(
						`<p class='discription_text'>Purging is successfully completed. Turn Off n2 gas.</p>`,
						3
					);
					show_panel(3);
				}
...
function load_system() {
	...
	for (let i = 0; i < all_compounds.length; i++) {
		sys.innerHTML += `<option value='${
			all_compounds[i].name
		}'>${all_compounds[i].name.toUpperCase()}</option>`;
	}
}
...
function on_system_select() {
	...
	canvas.addEventListener('click', a3_mouseclick_ebu_out);
	pp.showdescription(
		`<p class='discription_text'>Close outlet of ebulliometer.</p>`,
		3
	);
	show_panel(3);
}

function a3_mouseclick_ebu_in(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	console.log(x, y);
	...
	if (ebu_in.isinside(new Chemistry.Point(x, y))) {
		...
		if (ebu_in.color != 'red') {
			...
			pp.showdescription(
				`<p class='discription_text'>Turn on cold water pump.</p>`,
				3
			);
			show_panel(3);
			draw_seq_all();
		...
		var second_geo = new Chemistry.anim_image_y_dir_down(
			seq4_img,
			new Chemistry.Point(470, 440),
			594,
			701,
...
function a3_mouseclick_pump(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	console.log(x, y);
...
function a3_mouseclick_coil(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	console.log(x, y);

	let slider: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('stat-dd')
	);
	...
	if (heating_coil.isinside(new Chemistry.Point(x, y))) {
		...
		drops = new Chemistry.Custome_image(
			drop1,
			new Chemistry.Point(600, 80),
			288 * 0.6,
			360 * 0.6,
			canvas
		);

		var indicator = new Chemistry.Custome_image(
			temp_indi,
			new Chemistry.Point(150, 400),
			92 * 1.2,
			60 * 1.2,
			canvas
		);

		var dim = new Chemistry.Custome_image(
			dimmer,
			new Chemistry.Point(1050, 500),
			421,
			422,
			canvas
		);

		let temp = new Chemistry.Geo_Text(
			'Temp',
			new Chemistry.Point(113, 450),
			canvas
		);
		temp.font = '25vw Arial';

		point = new Chemistry.Custome_image(
			dial,
			new Chemistry.Point(1050, 500),
			178,
			236,
		...
		// drops.draw();
		pp.showdescription(
			`<p class='discription_text'>Move slider to increase or decrease heating rate.</p>`,
			3
		);
		show_panel(3);
	}
...
function get_values() {
	let compound: HTMLSelectElement = <HTMLSelectElement>(
		document.getElementById('compounds-dd')
	);
	let slider: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('stat-dd')
...
function calculate_Q0() {
	...
	let display: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('display-slider-value')
	);
	let n_val: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('display-n-value')
	);
	let h_val: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('display-dh-value')
	);
	...
	let temp_text = new Chemistry.Geo_Text(
		main_table_data[0][9].toFixed(2).toString(),
		new Chemistry.Point(113, 390),
		canvas
	);
